...
Future 객체는 작업 결과가 아니라 작업이 완료될 때까지 기다렸다가 최종 결과를 얻는데 사용 된다.
...
안녕하세요. 스프링을 개발하고있는 개발자입니다.
우연히 쓰레드 관련 영상을 찾다가 유튜브에서 부종민님이 스프링캠프에서 발표했던 '프로세스와 스레드, NIO 그리고 리액티브 스트림 (Reactive Stream)'내용이 유튜브에 올라와있어 그것을보고 궁금한점이 있어서 이렇게 메일 드립니다!
...
제가찾아본것은 setCorePoolSize 는 코어(CPU)의 갯수를 setMaxPoolSize 는 최대 생성 쓰레드 갯수를, setQueueCapacity는 쓰레드의 사용을 위해 대기되는 요청의 큐 사이즈로 알고있습니다. 그런데 쓰레드 갯수나 쓰레드 최대갯수를 어떤기준으로 값들을 정의해야하는걸까요? 만약 시간이 되신다면 답변주시면 감사하겠습니다(__).